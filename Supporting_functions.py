"""
Helper functions needed to conduct NMF. Mostly used to calculate sparse NMF error using only entries with values in them
Other helper functions include reversing min_max normalisation to get clinically meaningful error and cophenetic correlation calculation
"""
...
#Function to create folds if using bi-fold cross validation
...
#Average linkage
def coph_correlation(model, data, runs, rank):
    A = consensus(model, data, runs, rank)
...
def consensus(model, data, runs, rank):
    C_bar = np.zeros(shape=(data.shape[1],data.shape[1],1))
    for i in range(runs):
        C = np.zeros(shape=(data.shape[1],data.shape[1],1))
        model_run = model(n_components = rank, init = 'random', solver = 'mu', verbose = True, max_iter = 200)
        W = model_run.fit_transform(data)
        H = model_run.components_
        cluster = np.argmax(H, axis= 0)
        for j in range(C.shape[0]):
            for k in range(C.shape[1]):
                if cluster[k] == cluster[j]:
                    C[j,k] = 1
                    C[k,j] = 1
        C_bar = np.concatenate([C_bar, C], axis = -1)
    return np.mean(C_bar[:,:,1:], axis = -1)
...
#Reverse normalisation after fitting
def reverse_normalisation(data, original_data):
    data_2 = np.nan_to_num(original_data,nan=0.0, copy = True)
    w = np.sqrt(np.sum(data_2**2, axis = 0))
    output = data*w
    output[output == 0] = np.nan
    return pd.DataFrame(output, columns = original_data.columns, index = original_data.index)
